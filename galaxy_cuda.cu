...
unsigned int *h_hist_DR, *h_hist_DD, *h_hist_RR;
...
int main(int argc, char *argv[])
{
   ...
   h_real_rasc = (float *)calloc(INPUT_SIZE, sizeof(float));
   h_real_decl = (float *)calloc(INPUT_SIZE, sizeof(float));
   h_rand_rasc = (float *)calloc(INPUT_SIZE, sizeof(float));
   h_rand_decl = (float *)calloc(INPUT_SIZE, sizeof(float));
   CPUMemory += 4L * arr_size;

   h_hist_DR = (unsigned int *)calloc(BIN_COUNT, sizeof(unsigned int));
   h_hist_DD = (unsigned int *)calloc(BIN_COUNT, sizeof(unsigned int));
   h_hist_RR = (unsigned int *)calloc(BIN_COUNT, sizeof(unsigned int));
   CPUMemory += 3L * hist_arr_size;

   if ( parseargs_readinput(argc, argv, h_real_rasc, h_real_decl, h_rand_rasc, h_rand_decl) != 0 ) return(-1);

   // For your entertainment: some performance parameters of the GPU you are running your programs on!
   // if ( getDevice() != 0 ) return(-1);

   float *d_real_rasc; cudaMalloc(&d_real_rasc, arr_size);
   float *d_real_decl; cudaMalloc(&d_real_decl, arr_size);
   float *d_rand_rasc; cudaMalloc(&d_rand_rasc, arr_size);
   float *d_rand_decl; cudaMalloc(&d_rand_decl, arr_size);
   GPUMemory += 4L * arr_size;
   ...
   unsigned int *d_hist_DD; cudaMalloc(&d_hist_DD, hist_arr_size);
   unsigned int *d_hist_RR; cudaMalloc(&d_hist_RR, hist_arr_size);
   unsigned int *d_hist_DR; cudaMalloc(&d_hist_DR, hist_arr_size);
   GPUMemory += 3L * hist_arr_size;
   
   cudaMemcpy(d_real_rasc, h_real_rasc, arr_size, cudaMemcpyHostToDevice);
   cudaMemcpy(d_real_decl, h_real_decl, arr_size, cudaMemcpyHostToDevice);
   cudaMemcpy(d_rand_rasc, h_rand_rasc, arr_size, cudaMemcpyHostToDevice);
   cudaMemcpy(d_rand_decl, h_rand_decl, arr_size, cudaMemcpyHostToDevice);

   ArcToVector<<<BLOCK_COUNT, THREADS_PER_BLOCK>>>(d_real_decl, d_real_rasc, d_real_x, d_real_y, d_real_z, INPUT_SIZE);  // Real
   ArcToVector<<<BLOCK_COUNT, THREADS_PER_BLOCK>>>(d_rand_decl, d_rand_rasc, d_rand_x, d_rand_y, d_rand_z, INPUT_SIZE);  // Rand

   dim3 dimGrid(BLOCK_COUNT, BLOCK_COUNT);
   CountHist<<<dimGrid, THREADS_PER_BLOCK>>>(d_real_x, d_real_y, d_real_z, d_real_x, d_real_y, d_real_z, d_hist_DD, INPUT_SIZE); // DD
   CountHist<<<dimGrid, THREADS_PER_BLOCK>>>(d_rand_x, d_rand_y, d_rand_z, d_rand_x, d_rand_y, d_rand_z, d_hist_RR, INPUT_SIZE); // RR
   CountHist<<<dimGrid, THREADS_PER_BLOCK>>>(d_real_x, d_real_y, d_real_z, d_rand_x, d_rand_y, d_rand_z, d_hist_DR, INPUT_SIZE); // DR

   cudaMemcpy(h_hist_DD, d_hist_DD, hist_arr_size, cudaMemcpyDeviceToHost);
   cudaMemcpy(h_hist_RR, d_hist_RR, hist_arr_size, cudaMemcpyDeviceToHost);
   cudaMemcpy(h_hist_DR, d_hist_DR, hist_arr_size, cudaMemcpyDeviceToHost);

   int correct_sum = 1;
   if (check_correct_hist_sum(h_hist_DD, BIN_COUNT, RESULT_SIZE, "DD") == 0) correct_sum = 0;
   if (check_correct_hist_sum(h_hist_RR, BIN_COUNT, RESULT_SIZE, "RR") == 0) correct_sum = 0;
   if (check_correct_hist_sum(h_hist_DR, BIN_COUNT, RESULT_SIZE, "DR") == 0) correct_sum = 0;
   if (correct_sum != 1) {
   ...
   for ( int i = 0; i < BIN_COUNT; ++i ) {
      if ( h_hist_RR[i] > 0 ) {
          double omega =  (h_hist_DD[i]-2*h_hist_DR[i]+h_hist_RR[i])/((double)(h_hist_RR[i]));

          fprintf(outfil,"%6.3f\t%15lf\t%15u\t%15u\t%15u\n",((float)i)/BINS_PER_DEGREE, omega,
             h_hist_DD[i], h_hist_DR[i], h_hist_RR[i]);
   ...
   cudaFree(d_real_rasc); cudaFree(d_real_decl); cudaFree(d_rand_rasc); cudaFree(d_rand_decl);
   cudaFree(d_real_x); cudaFree(d_real_y); cudaFree(d_real_z); cudaFree(d_rand_x); cudaFree(d_rand_y); cudaFree(d_rand_z);
   cudaFree(d_hist_DD); cudaFree(d_hist_RR); cudaFree(d_hist_DR);
   free(h_real_decl); free(h_real_rasc); free(h_rand_decl); free(h_rand_rasc);
   free(h_hist_DR); free(h_hist_DD); free(h_hist_RR);
